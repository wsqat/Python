...
class download:
    ...
    def __init__(self):
        ...
        for ip in html.text.split('\n'):
            self.iplist.append(ip.replace('\r', ''))
    ...
    def get(self, url, timeout, proxy=None, num_retries=6): ##给函数一个默认参数proxy为空
        ...
        headers = {'User-Agent': UA,
                'Referer':'http://www.mzitu.com/'}  ##构造成一个完整的User-Agent （UA代表的是上面随机取出来的字符串哦）
...
# 使用代理获取response
# Xz = download() ##实例化
# print(Xz.get("mzitu.com", 3)) 
##打印headers
request = download()  ## add for mzitu4.py
down = download()  ## add for mzitu5.py 
