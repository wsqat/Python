...
class XiaoshuoPipeline(object):
    ...
    def __init__(self):
        ...
        # self.csvfile = open('cszw2.csv', 'ab')  # 小说 追加写入 diangdian
        self.csvfile = open('xiaoshuo.csv', 'ab')  # 小说 追加写入 cs
        ...
        # self.csvwriter.writerow(['novel_name', 'author', 'novelurl', 'serialstatus', 'serialnumber', 'category', 'collects','reviews'])

    def process_item(self, item, spider):
        row = [item['novel_name'], item['author'], item['novelurl'],item['serialstatus'], item['serialnumber'], item['category'],
                    item['collect_num_total'], item['click_num_total']]
